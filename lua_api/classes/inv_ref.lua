...
function InvRef:_remove()
	...
	if invtype == "player" then
		modtest.api.player_inventories[location.name] = nil
	elseif invtype == "node" then
		modtest.api.node_inventories[core.hash_node_position(location.pos)] = nil
	elseif invtype == "detached" then
		modtest.api.detached_inventories[location.name] = nil
	end
	self._removed = true
end
